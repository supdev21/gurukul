...
jQuery(function ($) {
	...
	window.opener.postMessage(JSON.stringify(userData), 'http://gurukul.definedge.local:9096/checkout');

	// Listen for messages from the popup window
	window.addEventListener('message', function(event) {
	    // Check if the message is from the expected origin
	    if (event.origin === 'http://gurukul.definedge.local:9096/checkout') {
