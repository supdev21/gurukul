...
<div class="box-info-white">
	<h3 class="title">
		<?php
			$title = esc_html__( 'Course Content', 'educrat' );
	...
	<?php if ( $topics->have_posts() ) : ?>
		...
		<?php while ( $topics->have_posts() ) : ?>
			...
			<div class="tutor-accordion-item">
				<h4 class="tutor-accordion-item-header<?php echo esc_attr($index == 1 ? ' is-active': ''); ?>">
					<?php the_title(); ?>
				...
				<?php if ( $topic_contents->have_posts() ) : ?>
					<div class="tutor-accordion-item-body" style="<?php echo esc_attr($index != 1 ? 'display: none;': ''); ?>">
								...
								<?php while ( $topic_contents->have_posts() ) : ?>
									...
									<li class="tutor-course-content-list-item">
										<div class="tutor-d-flex tutor-align-center">
											<span class="tutor-course-content-list-item-icon <?php echo trim($topic_content_icon); ?> tutor-mr-12"></span>
													...
													if ( $is_enrolled || ( get_post_meta( $course_id, '_tutor_is_public_course', true ) == 'yes' && ! tutor_utils()->is_course_purchasable( $course_id ) ) ) {
														...
														if ( $countdown ) {
															if ( $zoom_meeting->is_expired ) {
																$lesson_title .= '<span class="tutor-zoom-label">' . esc_html__( 'Expired', 'educrat' ) . '</span>';
															} elseif ( $zoom_meeting->is_started ) {
																$lesson_title .= '<span class="tutor-zoom-label tutor-zoom-live-label">' . esc_html__( 'Live', 'educrat' ) . '</span>';
															}
															$lesson_title .= $countdown;
														}
	
														echo trim($lesson_title);
													} else {
										...
										<div>
											<span class="tutor-course-content-list-item-duration tutor-fs-7 tutor-color-muted">
												<?php echo trim($play_time ? tutor_utils()->get_optimized_duration( $play_time ) : ''); ?>
											</span>
											<span class="tutor-course-content-list-item-status <?php echo esc_attr($is_locked ? 'tutor-icon-lock-line' : 'tutor-icon-eye-line'); ?> tutor-color-muted tutor-ml-20" area-hidden="true"></span>
										</div>
									</li>
								<?php endwhile; ?>
							</ul>
