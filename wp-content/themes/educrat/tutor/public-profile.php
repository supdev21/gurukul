...
<?php $user_identifier = $is_instructor ? 'tutor-instructor' : 'tutor-student'; ?>
	<div <?php tutor_post_class( 'tutor-wrap-parent tutor-full-width-student-profile tutor-page-wrap tutor-user-public-profile tutor-user-public-profile-' . $profile_layout . ' ' . $user_identifier ); ?> >
			...
			<div class="pp-area">
				...
				<div class="profile-name tutor-color-white">
					<div class="profile-rating-media content-for-mobile">
						<?php echo trim($rating_content); ?>
						<div class="tutor-social-container content-for-desktop">
							<?php echo trim($social_media); ?>
						</div>
					</div>

					<h3><?php echo trim($get_user->display_name); ?></h3>
					<?php
					if ( $is_instructor ) {
						...
						?>
							<span class="pre">
								<i class="flaticon-play-1 pre"></i>
								<span><?php echo trim($course_count); ?></span> 
								<?php $course_count > 1 ? esc_html_e( 'Courses', 'educrat' ) : esc_html_e( 'Course', 'educrat' ); ?>
							</span>
							<span class="pre"></span>
							<span class="pre">
								<i class="flaticon-online-learning-5 pre"></i>
								<span><?php echo trim($student_count); ?></span> 
								<?php $student_count > 1 ? esc_html_e( 'Students', 'educrat' ) : esc_html_e( 'Student', 'educrat' ); ?>
							</span>
						<?php
					} else {
						...
						?>
							<span class="pre">
								<i class="flaticon-play-1 pre"></i>
								<span><?php echo trim($enrol_count); ?></span> 
							<?php $enrol_count > 1 ? esc_html_e( 'Courses Enrolled', 'educrat' ) : esc_html_e( 'Course Enrolled', 'educrat' ); ?>
							</span>
							<span class="pre"></span>
							<span class="pre">
								<i class="flaticon-online-learning-5 pre"></i>
								<span><?php echo trim($complete_count); ?></span> 
							<?php $complete_count > 1 ? esc_html_e( 'Courses Completed', 'educrat' ) : esc_html_e( 'Course Completed', 'educrat' ); ?>
							</span>
							<?php
					}
					?>
		...
		<div class="tutor-container clearfix">
			<div class="tutor-user-profile-sidebar">
			</div>
			<div class="tutor-user-profile-content tutor-d-block tutor-mt-sm-72">
				<h3><?php esc_html_e( 'Biography', 'educrat' ); ?></h3>
				<?php tutor_load_template( 'profile.bio' ); ?>
				
				<?php
				if ( $is_instructor ) {
					?>
						<h3><?php esc_html_e( 'Courses', 'educrat' ); ?></h3>
						<?php
							add_filter(
								'courses_col_per_row',
								function() {
