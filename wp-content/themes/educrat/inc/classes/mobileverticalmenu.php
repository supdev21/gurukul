...
if ( !class_exists("Educrat_Mobile_Vertical_Menu") ) {
    class Educrat_Mobile_Vertical_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'educrat');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'educrat');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'educrat');
                        break;
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            if ( $apus_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    $post = $posts[0];
                    $content = apply_filters( 'educrat_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
