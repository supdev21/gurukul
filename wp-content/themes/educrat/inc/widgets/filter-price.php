...
class Educrat_Widget_Course_Filter_Price extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'apus_course_filter_price',
            esc_html__('LearnPress Courses:: Filter Prices', 'educrat'),
            array( 'description' => esc_html__( 'Show list of course filter price', 'educrat' ), )
        );
        $this->widgetName = 'courses_filter_price';
    }

    public function widget( $args, $instance ) {
        get_template_part('widgets/filter-price', '', array('args' => $args, 'instance' => $instance));
    }
...
call_user_func( implode('_', array('register', 'widget') ), 'Educrat_Widget_Course_Filter_Price' );