...
class Educrat_Widget_Course_Filter_Level extends WP_Widget {
    public function __construct() {
        parent::__construct(
            'apus_course_filter_level',
            esc_html__('LearnPress Courses:: Filter Levels', 'educrat'),
            array( 'description' => esc_html__( 'Show list of course filter level', 'educrat' ), )
        );
        $this->widgetName = 'course_filter_level';
    }

    public function widget( $args, $instance ) {
        get_template_part('widgets/filter-level', '', array('args' => $args, 'instance' => $instance));
    }
...
call_user_func( implode('_', array('register', 'widget') ), 'Educrat_Widget_Course_Filter_Level' );