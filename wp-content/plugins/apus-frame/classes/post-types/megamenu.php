...
class Apus_Frame_PostType_Megamenu {
  	...
  	public static function register_post_type() {
	    $labels = array(
			'name'                  => __( 'Megamenu Profiles', 'apus-frame' ),
			'singular_name'         => __( 'Megamenu Profile', 'apus-frame' ),
			'add_new'               => __( 'Add New Megamenu Profile', 'apus-frame' ),
			'add_new_item'          => __( 'Add New Megamenu Profile', 'apus-frame' ),
			'edit_item'             => __( 'Edit Megamenu Profile', 'apus-frame' ),
			'new_item'              => __( 'New Megamenu Profile', 'apus-frame' ),
			'all_items'             => __( 'All Megamenu Profiles', 'apus-frame' ),
			'view_item'             => __( 'View Megamenu Profile', 'apus-frame' ),
			'search_items'          => __( 'Search Megamenu Profile', 'apus-frame' ),
			'not_found'             => __( 'No Megamenus found', 'apus-frame' ),
			'not_found_in_trash'    => __( 'No Megamenus found in Trash', 'apus-frame' ),
			'parent_item_colon'     => '',
			'menu_name'             => __( 'Megamenu Profiles', 'apus-frame' ),
	    );
  	...
  	public static function script() {
  		wp_enqueue_media();
  		wp_enqueue_script( 'apus-upload-image', APUS_FRAME_URL . 'assets/upload.js', array( 'jquery', 'wp-pointer' ), APUS_FRAME_VERSION, true );
  	}
  	
  	public static function nav_edit_walker() {
		$walker = 'Apus_Frame_Megamenu_Config';
		if ( ! class_exists( $walker ) ) {
			require_once APUS_FRAME_DIR . '/classes/class-apus-megamenu.php';
		}
	...
	?>
		<p class="field-icon-font description description-wide">   
			<label for="edit-menu-item-icon-font-<?php echo esc_attr($item_id); ?>"><?php _e( 'Icon Font (Awesome):', 'apus-frame' ); ?> <br>
				<input type="text"  name="menu-item-apus_icon_font[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr($icon_font); ?>">
			</label>
			<br>
			<span><?php _e('This support display icon from FontAwsome, Please click <a href="//fontawesome.com/v4.7.0/icons" target="_blank"> <b>here</b></a> to see the list.', 'apus-frame');?></span>
		</p>
		<p class="field-icon-image description description-wide">   
			<label for="edit-menu-item-icon-image-<?php echo esc_attr($item_id); ?>"><?php _e( 'Icon Image:', 'apus-frame' ); ?></label>
			...
			<span><?php _e('You can use Icon Font or Icon Image', 'apus-frame');?></span>
		</p>
		...
		<p class="field-addclass description description-wide">
			<label for="edit-menu-item-apus_mega_profile-<?php echo esc_attr($item_id); ?>"> 
			  <?php _e( 'Megamenu Profile' ); ?> <br>
			   	<select name="menu-item-apus_mega_profile[<?php echo esc_attr($item_id); ?>]">
				    <option value=""><?php _e( 'Disable', 'apus-frame' ); ?></option>
			...
			<a href="<?php echo  esc_url( admin_url( 'edit.php?post_type=apus_megamenu') ); ?>" target="_blank" title="<?php _e( 'Sub Megamenu Management', 'apus-frame' ); ?>"><?php _e( 'Sub Megamenu Management', 'apus-frame' ); ?></a>
			<span><?php _e( 'If enabled megamenu, its submenu will be disabled', 'apus-frame' ); ?></span>
		</p>

		<p class="field-apus_width description description-wide">   
			<label for="edit-menu-item-apus_width-<?php echo esc_attr($item_id); ?>"><?php _e( 'Width:', 'apus-frame' ); ?> <br>
		...
		<?php 
			$aligns = array(
			    'left' => __('Left', 'apus-frame'),
			    'right' => __('Right', 'apus-frame'),
			    'fullwidth' => __('Fullwidth', 'apus-frame')
			); 
		?> 
		<p class="field-apus_alignment description description-wide">   
			<label for="edit-menu-item-apus_alignment-<?php echo esc_attr($item_id); ?>"><?php _e( 'Alignment:', 'apus-frame' ); ?> <br>
    ...
    ?>
        <p class="field-addclass description description-wide">
            <label for="edit-menu-item-apus_text_label-<?php echo esc_attr($item_id); ?>">
                <?php  echo __( 'Label', 'apus-frame' ); ?><br />
                <select name="menu-item-apus_text_label[<?php echo esc_attr($item_id); ?>]">
                  <option value="" <?php selected( esc_attr($val), '' ); ?>><?php _e('None', 'apus-frame'); ?></option>
                  <option value="label_new" <?php selected( esc_attr($val), 'label_new' ); ?>><?php _e('New', 'apus-frame'); ?></option>
                  <option value="label_hot" <?php selected( esc_attr($val), 'label_hot' ); ?>><?php _e('Hot', 'apus-frame'); ?></option>
                  <option value="label_featured" <?php selected( esc_attr($val), 'label_featured' ); ?>><?php _e('Featured', 'apus-frame'); ?></option>
                </select>
            </label>
        </p>
    <?php
...
Apus_Frame_PostType_Megamenu::init();