...
class OptInPage implements \FcfVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function __construct($plugin_file, $plugin_slug, $shop = null)
    {
        ...
        $this->shop = $shop ?? new \FcfVendor\WPDesk\Tracker\Shop('');
    }
    ...
    public function output()
    {
        ...
        if (isset($_GET['shop_url'])) {
            $shop = new \FcfVendor\WPDesk\Tracker\Shop(\sanitize_text_field(\wp_unslash($_GET['shop_url'])));
        } else {
            $shop = $this->shop;
        }
        ...
        $renderer = new \FcfVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \FcfVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
